...
for(i in 1:length(file.names)){
  ...
  ggsave(gsub(" ", "", paste(file.names[i], '_seqsxread_hist.png')), width = 6, height = 5, units = 'cm',
         path = 'E:/oscar/Documents/TFM/5.Filter_and_ReplicateLibraries/5.1.Replicates_Prefilter/5.1.2.Seqxreads')
  rm(file)
  rm(reads)
  rm(seqs)
  rm(file_df)
